...
function getBasketType(tokens: BasketInfo["tokens"]): string {
  ...
  if (config.length === 3) {
    const hasBTC40 = config.some((c) => c.symbol.includes("BTC") && c.pct === 40);
    const hasETH40 = config.some((c) => c.symbol.includes("ETH") && c.pct === 40);
    const hasPYUSD20 = config.some((c) => c.symbol.includes("PYUSD") && c.pct === 20);
...
function AssetIcon({ asset }: { asset: string }) {
  ...
  if (asset === "BTC" || asset.includes("BTC")) {
    return (
      <svg className="asset-icon" viewBox="0 0 20 20" aria-label="BTC">
        <circle cx="10" cy="10" r="9" fill="#f4b154" stroke={ink} strokeWidth="2" />
        <text x="10" y="12" textAnchor="middle" fontWeight={900} fontSize="9" fill={ink}>
          ₿
        </text>
      </svg>
    );
  ...
  if (asset === "ETH" || asset.includes("ETH")) {
    return (
      <svg className="asset-icon" viewBox="0 0 20 20" aria-label="ETH">
        <polygon points="10,2 15,10 10,12 5,10" fill="#8ec5ff" stroke={ink} strokeWidth="2" />
        <polygon points="10,18 15,10 10,12 5,10" fill="#b8dcff" stroke={ink} strokeWidth="2" />
      </svg>
    );
  }
  // PYUSD or generic
  return (
    <svg className="asset-icon" viewBox="0 0 20 20" aria-label={asset}>
      <circle cx="10" cy="10" r="9" fill={ASSET_COLORS[asset] || "#61d3a5"} stroke={ink} strokeWidth="2" />
      <text x="10" y="12" textAnchor="middle" fontWeight={900} fontSize="9" fill={ink}>
        {asset[0]}
      </text>
    </svg>
...
export default function Profile({ onLogout }: ProfileProps) {
      ...
      const basketsWithValues = await Promise.all(
        baskets.map(async (basket) => {
          try {
            ...
            console.error(`Error calculating value for basket ${basket.address}:`, err);
            return basket;
          }
        })
      );
  ...
  const handleRebalance = async (basketAddress: string) => {
    try {
      ...
      // Refresh basket data after a delay (will get new balances and recalculate value)
      setTimeout(async () => {
        console.log("🔄 Refreshing basket data...");
        if (walletAddress) {
          await loadBasketData(walletAddress);
        }
      }, 10000); // Wait 10 seconds for tx to confirm
    } catch (err) {
      ...
      <div className="profile-header">
        ...
        <div className="profile-info">
          <div className="greeting">
            {getGreeting()}, <span className="nickname-highlight">{nickname}</span>! 🥔
          </div>
          <p className="welcome-text">Welcome to your Potato Finance dashboard</p>
          <p className="wallet-address">
            Wallet: {walletAddress.slice(0, 6)}...{walletAddress.slice(-4)}
          </p>
          {lastUpdated && (
            <p className="last-updated">Last updated: {lastUpdated.toLocaleTimeString()} • Auto-refreshing every 10s</p>
          )}
        </div>
      </div>
      ...
      <section className="deposit-instructions">
        <div className="instruction-card">
          ...
          <p className="instruction-text">
            Send any supported token directly to your basket endpoint below. Your basket will automatically rebalance to
            maintain your target allocations.
          </p>
          <div className="pyusd-highlight">
            <strong>💙 PayPal Users:</strong> You can easily send PYUSD directly from your PayPal app! Just copy the ENS
            address from your basket and use it as the recipient address.
          </div>
        </div>
      </section>
      ...
      <section className="profile-section">
        ...
        {userBaskets.length === 0 ? (
          <div className="empty-state">
            <p>You don't have any baskets yet. Send funds to your endpoints to get started!</p>
          </div>
        ) : (
          <div className="user-baskets-grid">
            {userBaskets.map((basket) => {
              ...
              const basketData = SUPPORTED_BASKETS.find((b) => b.name === basketType);
                ...
                <article key={basket.address} className="user-basket-card">
                    ...
                    {basketData && (
                      <div className="metrics">
                        <span className="chip-positive">
                          <span className="value">+{basketData.improvement.toFixed(2)}%</span> vs HODL
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  {basket.totalValue !== undefined && (
                    <div className="basket-total-value">
                      ...
                      <span className="value-amount">${basket.totalValue.toFixed(2)}</span>
                    </div>
                  )}
                        ...
                        <div className="token-info">
                          ...
                          <div className="token-details">
                            <div className="token-name">
                              {token.symbol}
                              <span className="token-percentage">{token.percentage}%</span>
                            </div>
                            <div className="token-full-name">{token.name}</div>
                          </div>
                        </div>
                        <div className="token-balance">
                          <div className="balance-value">{token.formattedBalance}</div>
                  ...
                  <div className="card-actions">
                    ...
                    >
                      {rebalancing === basket.address ? "Rebalancing..." : "Rebalance"}
                    </button>
                  </div>
                </article>
              );
